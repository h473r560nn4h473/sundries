...
import java.util.InputMismatchException;    // 잘못된 입력값으로 발생할 수 있는 InputMismatchException 예외 처리
...
public class Print_001 {
  ...
  private static void listStuff() {
    ...
    System.out.println("\n---\n");
    System.out.println("할일을 저장하려면 메인메뉴에서 프로그램 종료를 선택해서 종료해야 합니다.");
  ...
  private static void toggleStuff(Scanner scanner) {    // Scanner 객체를 매개변수로 받아 할일의 체크 상태를 변경한다
    int stuffNumber = -1; // 사용자가 입력한 번호를 읽어와 stuffNumber 변수에 저장한다. -1로 초기화하는 이유는 유효한 입력을 받기 전까지 taskNumber가 유효하지 않은 상태임을 나타내기 위해서이다.
    while(true) {
      try {
        System.out.println("체크/체크해제할 할일 번호를 입력하세요:");
        stuffNumber = scanner.nextInt();
        if (stuffNumber < 1 || stuffNumber > 10) {
            System.out.println("할일은 최대 10개까지만 저장할 수 있습니다.");
            continue;
        }
        stuffNumber--; // 리스트 인덱스는 0부터 시작하므로 1을 빼준다.
        break;
      } catch(InputMismatchException e) {
        System.out.println("잘못된 입력입니다. 숫자를 입력하세요.");
        scanner.next(); // 직전에 입력한 값이 버퍼에 남아있으므로 한번 소비해줘야 한다.
      }
    }
